...
function extractFunctionBody(code, funcIndex, allFunctions, currentIdx) {
  ...
  function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== '.git') {
        walk(fullPath);
      } else if (entry.isFile() && entry.name.endsWith(extension) && !entry.name.endsWith('.summary.json')) {
...
function generateSummary(filePath, functionSummaries) {
...
async function summarizeFunctionsWithLLM(ollamaUrl, model, filePath) {
  ...
  for (let i = 0; i < functions.length; i++) {
    ...
    try {
      const response = await queryOllama(ollamaUrl, model,
...
async function analyzeFolderWithLLM(ollamaUrl, model, folderPath) {
  const files = scanDirectory(folderPath);
  const summaries = [];
  for (const file of files) {
    const funcSummaries = await summarizeFunctionsWithLLM(ollamaUrl, model, file);
