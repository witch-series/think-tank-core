...
const startCLI = (context) => {
  const { taskManager, getConfig, getClient, log, restart, createTaskFn, doubleCheckFn, saveKnowledgeFn, analyzeFolderWithLLMFn, scanDirectoryFn, getAnalyzeLoop } = context;
    ...
    switch (cmd) {
      ...
      case 'inject':
        ...
        taskManager.prioritize(createTaskFn(`cli:research`, async () => {
          ...
          const researchDb = require('path').resolve(context.root, 'brain', 'research');
          if (result.accepted) {
            saveKnowledgeFn(researchDb, 'research', {
              topic: rest,
      ...
      case 'analyze': {
        ...
        const loop = getAnalyzeLoop ? getAnalyzeLoop() : null;
        if (loop) {
          const files = scanDirectoryFn(absPath, '.js');
          for (const f of files) loop._enqueue(f);
          console.log(`Analyze queued: ${files.length} files from ${absPath}`);
        } else {
