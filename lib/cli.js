...
function startCLI(context) {
  const { taskManager, getConfig, log, restart, createTaskFn, doubleCheckFn, saveKnowledgeFn, analyzeFolderWithLLMFn, scanDirectoryFn } = context;
    ...
    switch (cmd) {
      ...
      case 'inject':
        ...
        taskManager.prioritize(createTaskFn(`cli:research`, async () => {
          ...
          const result = await doubleCheckFn(config.ollama.url, config.ollama.model, rest);
      ...
      case 'analyze':
        ...
        taskManager.prioritize(createTaskFn(`cli:analyze:${rest}`, async () => {
          ...
          return analyzeFolderWithLLMFn(config.ollama.url, config.ollama.model, absPath);
        }));
