...
class OllamaClient {
  ...
  /**
   * Query the Ollama API and parse the response as JSON.
   * Retries with a simplified re-ask prompt on JSON parse failure.
   * @param {string} prompt - The user prompt
   * @param {string} [system] - System prompt
   * @param {object} [options] - { model, jsonRetries: number }
   * @returns {Promise<{response: string, parsed: object|null}>}
   */
  async queryForJson(prompt, system, options = {}) {
    const maxAttempts = options.jsonRetries || 2;
    let currentPrompt = prompt;
    let currentSystem = system;

    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const response = await this.query(currentPrompt, currentSystem, options);
      ...
      // On failure, retry with a re-ask prompt containing the broken output
      if (attempt < maxAttempts - 1) {
        this._notifyThinking(true, `JSON parse failed, retrying (${attempt + 1}/${maxAttempts})...`);
        currentPrompt =
          `あなたの前回の出力はJSON形式ではありませんでした。以下の内容をJSON形式だけで出力し直してください。説明文は不要です。\n\n前回の出力:\n${text.slice(0, 500)}`;
        currentSystem = 'JSONのみ出力してください。余計な説明は不要です。';
      }
    }
