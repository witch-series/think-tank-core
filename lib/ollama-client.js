...
const { fetchWithRetry } = require('../explorers/crawler');
...
class OllamaClient {
  ...
  async query(prompt, system, options = {}) {
    ...
    // Try all URLs in case of failure
    ...
    for (let attempt = 0; attempt < totalAttempts; attempt++) {
      ...
      try {
        const res = await fetchWithRetry(`${currentUrl}/api/generate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: payload,
          onRetry: (retryAttempt, max, err, delay) => {
            this._notifyThinking(true, `Connection error (${err.code}), retry ${retryAttempt}/${max} in ${delay / 1000}s...`);
          }
        });
        ...
        const switched = this._recordFailure();
        if (switched) {
          // Immediately try the new URL
          continue;
        }

        // If on the same URL, fetchWithRetry already did its retries,
        // so this counts as the final failure for this URL
