...
const runSetup = async (configPath) => {
  ...
  // Ollama
  console.log('-- Ollama --');
  console.log('  (複数URLはスペース区切りで設定可能。例: http://host1:11434 http://host2:11434)');
  const ollamaUrl = await ask(rl, 'Ollama URL(s)', defaults.ollama?.url || 'http://localhost:11434');
  ...
  // Final Goal
  console.log('\n-- Final Goal (最終目標) --');
  console.log('  ※ この目標はシステムの全活動の方向性を決定します。一度設定すると自動では変更されません。');
  const finalGoal = await ask(rl,
    '最終目標を入力してください',
    defaults.finalGoal || '');
  ...
  const config = {
    ...
    summaryExtension: defaults.summaryExtension || '.summary.md'
  };
